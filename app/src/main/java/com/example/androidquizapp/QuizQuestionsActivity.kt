...
class QuizQuestionsActivity : AppCompatActivity(), View.OnClickListener {
    ...
    private fun setQuestion() {
        ...
        "$mCurrentPosition/${progressBar?.max?.minus(1)}".also { tvProgress?.text = it }// Setting up the progress text
    ...
    private fun answerView(answer: Int, drawableView: Int) {
        when (answer) {
            1 -> { tvOptionOne?.background = ContextCompat.getDrawable(this@QuizQuestionsActivity, drawableView) }
            2 -> { tvOptionTwo?.background = ContextCompat.getDrawable(this@QuizQuestionsActivity, drawableView) }
            3 -> { tvOptionThree?.background = ContextCompat.getDrawable(this@QuizQuestionsActivity, drawableView) }
            4 -> { tvOptionFour?.background = ContextCompat.getDrawable(this@QuizQuestionsActivity, drawableView) }
        }
    }
